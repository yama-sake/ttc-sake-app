...
// keys: { geminiKey, visionKey }
export async function readLabel(imageBase64, { geminiKey, visionKey } = {}) {
  ...
  if (geminiKey) {
    try {
      ...
      console.error('[labelReader] Gemini失敗、Visionにフォールバック:', e.message);
    }
  }

  if (visionKey) {
    return await readWithVision(imageBase64, visionKey);
  }

  throw new Error('APIキー未設定です。GEMINI_API_KEY または VISION_API_KEY を環境変数に設定してください。');
}
